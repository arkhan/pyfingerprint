...
class PyFingerprint(object):
    ...
    def __init__(self, port='/dev/ttyUSB0', baudRate=57600, address=0xFFFFFFFF, password=0x00000000):
        ...
        if (os.path.exists(port) == False):
            raise ValueError('The fingerprint sensor port "' +
                             port + '" was not found!')
        ...
        self.__serial = serial.Serial(
            port=port, baudrate=baudRate, bytesize=serial.EIGHTBITS, timeout=2)
    ...
    def __writePacket(self, packetType, packetPayload):
        ...
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(FINGERPRINT_STARTCODE, 8)))
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(FINGERPRINT_STARTCODE, 0)))

        self.__serial.write(utilities.byteToString(
            utilities.rightShift(self.__address, 24)))
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(self.__address, 16)))
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(self.__address, 8)))
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(self.__address, 0)))
        ...
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(packetLength, 8)))
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(packetLength, 0)))

        # The packet checksum = packet type (1 byte) + packet length (2 bytes)
        # + payload (n bytes)
        packetChecksum = packetType + \
            utilities.rightShift(packetLength, 8) + \
            utilities.rightShift(packetLength, 0)
        ...
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(packetChecksum, 8)))
        self.__serial.write(utilities.byteToString(
            utilities.rightShift(packetChecksum, 0)))
    ...
    def __readPacket(self):
        ...
        while (True):
            ...
            if (i >= 12):
                ...
                # Condition: index counter < packet payload length + packet
                # frame
                ...
                # Calculate checksum:
                # checksum = packet type (1 byte) + packet length (2 bytes) +
    ...
    def setSystemParameter(self, parameterNumber, parameterValue):
        ...
        # Validate the security level parameter
            ...
            if (parameterValue < 1 or parameterValue > 5):
                raise ValueError(
                    'The given security level parameter is invalid!')
        ...
        # Validate the package length parameter
            ...
            if (parameterValue < 0 or parameterValue > 3):
                raise ValueError(
                    'The given package length parameter is invalid!')
    ...
    def getSystemParameters(self):
        ...
        if (receivedPacketPayload[0] == FINGERPRINT_OK):
            ...
            statusRegister = utilities.leftShift(
                receivedPacketPayload[1], 8) | utilities.leftShift(receivedPacketPayload[2], 0)
            systemID = utilities.leftShift(
                receivedPacketPayload[3], 8) | utilities.leftShift(receivedPacketPayload[4], 0)
            storageCapacity = utilities.leftShift(
                receivedPacketPayload[5], 8) | utilities.leftShift(receivedPacketPayload[6], 0)
            securityLevel = utilities.leftShift(
                receivedPacketPayload[7], 8) | utilities.leftShift(receivedPacketPayload[8], 0)
            deviceAddress = ((receivedPacketPayload[9] << 8 | receivedPacketPayload[
                             10]) << 8 | receivedPacketPayload[11]) << 8 | receivedPacketPayload[12]  # TODO
            packetLength = utilities.leftShift(receivedPacketPayload[
                                               13], 8) | utilities.leftShift(receivedPacketPayload[14], 0)
            baudRate = utilities.leftShift(receivedPacketPayload[
                                           15], 8) | utilities.leftShift(receivedPacketPayload[16], 0)
    ...
    def getTemplateIndex(self, page):
        ...
        if (receivedPacketPayload[0] == FINGERPRINT_OK):
            ...
            for pageElement in pageElements:
                # Test every bit (bit = template position is used indicator) of
                # a table page element
                for p in range(0, 7 + 1):
                    positionIsUsed = (
                        utilities.bitAtPosition(pageElement, p) == 1)
    ...
    def downloadImage(self, imageDestination):
        ...
        if (os.access(destinationDirectory, os.W_OK) == False):
            raise ValueError('The given destination directory "' +
                             destinationDirectory + '" is not writable!')
        ...
        while (receivedPacketType != FINGERPRINT_ENDDATAPACKET):
            ...
            for i in range(0, len(receivedPacketPayload)):
                ...
                # Thanks to Danylo Esterman <soundcracker@gmail.com> for the
                # "multiple with 17" improvement:
